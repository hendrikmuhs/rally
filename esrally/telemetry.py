...
class TransformStats(TelemetryDevice):
    ...
    def __init__(self, telemetry_params, clients, metrics_store):
        ...
        if self.sample_interval <= 0:
            raise exceptions.SystemSetupError(
                "The telemetry parameter 'transform-stats-sample-interval' must be greater than zero but was {}.".format(self.sample_interval))
        self.specified_cluster_names = self.clients.keys()
    ...
    def on_benchmark_start(self):
        recorder = []
        for cluster_name in self.specified_cluster_names:
            recorder = TransformStatsRecorder(cluster_name, self.clients[cluster_name], self.metrics_store, self.sample_interval,
...
class TransformStatsRecorder:
    ...
    def record_stats_per_transform(self, name, stats):
        ...
        self.metrics_store.put_value_cluster_level("transform_search_time_in_ms", stats.get("search_time_in_ms", 0), "ms")
        self.metrics_store.put_value_cluster_level("transform_index_time_in_ms", stats.get("index_time_in_ms", 0), "ms")
        self.metrics_store.put_value_cluster_level("transform_processing_time_in_ms", stats.get("processing_time_in_ms", 0), "ms")
